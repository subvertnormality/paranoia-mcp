...
An MCP server that gets a second, adversarial opinion on your code changes and plans from ChatGPT (GPT-5.6).
...
- Python 3.11+
- An OpenAI API key with access to GPT-5.6 (either `Chat completions` or `Responses` endpoint scope — the server uses `/v1/responses`)
- `git` on `PATH`
...
|---|---|---|
| `PARANOIA_MODEL` | `gpt-5.6-sol` | Which OpenAI model to call |
| `PARANOIA_TOKEN_BUDGET` | `250000` | Input token budget (gpt-5.6 long-context pricing starts >272K input; leave ~22K for response + reasoning) |
